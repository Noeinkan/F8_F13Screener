...
- Telegram HTML headline per filing alert, composed by `format_headline_alert` in `src/core/report_builder.py` and sent by `TelegramNotifier.send_filing_alert`: fund name, quarter, Italian-formatted date, amendment flag, change counts, the single biggest move in dollars, filer name, plus inline buttons to the dashboard Compare tab and EDGAR. During a filing wave the alerts are folded into a digest by `src/cli/notify_reporter.py`.
